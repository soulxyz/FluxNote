...
try:
    ...
    def get_free_port(start_port=5001):
        """
        Finds a free TCP port starting at the given port number.
        
        Attempts to bind to localhost ports from start_port up to 65535 in increments of 10 and returns the first port that can be bound. If no free port is found, returns the original start_port.
        
        Parameters:
            start_port (int): Port number to start scanning from.
        
        Returns:
            int: A port number that is available for binding, or start_port if none was found.
        """
        port = start_port
        while port <= 65535:
